...
public class PermutationDP extends JPanel {
	...
	public PermutationDP() {
		...
		top.setBorder(BorderFactory.createTitledBorder(topBD, "Thông tin - Giải mã Permutation"));
		...
		ActionListener buttonListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (e.getActionCommand().equals("Choose File")) {
					...
					int userChoice = textFileChooser.showOpenDialog(PermutationDP.this);
				...
				if (e.getActionCommand().equals("Load key")) {
					...
					int userChoice = keyFileChooser.showOpenDialog(PermutationDP.this);
					if (userChoice == JFileChooser.APPROVE_OPTION) {
				...
				if (e.getActionCommand().equals("Decrypt")) {
					if (ctTextField.getText().isBlank() || keyTextField.getText().isBlank()) {
						...
						int userChoice = dirChooser.showSaveDialog(PermutationDP.this);
						if (userChoice == JFileChooser.APPROVE_OPTION) {
							...
							try {
								per.decrypt(text, key, destDir);

								// Show result to Text Area
								txtArea.setText("Result");
								txtArea.append("\n" + per.getDecryptedString());
							} catch (Exception e1) {
								e1.printStackTrace();
							}
						}
