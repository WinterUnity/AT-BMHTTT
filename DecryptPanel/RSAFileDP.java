...
public class RSAFileDP extends JPanel {
	...
	public RSAFileDP() {
		...
		top.setBorder(BorderFactory.createTitledBorder(topBD, "Thông tin - Giải mã RSA - File"));
		...
		ActionListener buttonListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (e.getActionCommand().equals("Choose File")) {
					...
					int userChoice = textFileChooser.showOpenDialog(RSAFileDP.this);
				...
				if (e.getActionCommand().equals("Load key")) {
					...
					int userChoice = keyFileChooser.showOpenDialog(RSAFileDP.this);
					if (userChoice == JFileChooser.APPROVE_OPTION) {
				...
				if (e.getActionCommand().equals("Decrypt")) {
					if (ctTextField.getText().isBlank() || keyTextField.getText().isBlank()) {
						...
						if (keyUploaded != true) {
							...
							int userChoice = dirChooser.showSaveDialog(RSAFileDP.this);
							if (userChoice == JFileChooser.APPROVE_OPTION) {
								...
								// Decrypting
								try {
									rsa.decryptFile(textFile, privateKey, destDir);
