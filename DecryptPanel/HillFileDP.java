...
public class HillFileDP extends JPanel {
	...
	public HillFileDP() {
		...
		top.setBorder(BorderFactory.createTitledBorder(topBD, "Thông tin - Giải mã Hill - File"));
		...
		ActionListener buttonListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (e.getActionCommand().equals("Choose File")) {
					...
					int userChoice = textFileChooser.showOpenDialog(HillFileDP.this);
				...
				if (e.getActionCommand().equals("Load key")) {
					...
					int userChoice = keyFileChooser.showOpenDialog(HillFileDP.this);
					if (userChoice == JFileChooser.APPROVE_OPTION) {
				...
				if (e.getActionCommand().equals("Decrypt")) {
					if (ctTextField.getText().isBlank() || keyTextField1.getText().isBlank()
						...
						if (fileUploaded != true) {
							...
							int userChoice = dirChooser.showSaveDialog(HillFileDP.this);
							if (userChoice == JFileChooser.APPROVE_OPTION) {
								...
								Hill hill = new Hill();
								try {
									hill.decryptFile(textFile, key, destDir);
