...
public class SubstitutionDecryptPanel extends JPanel {
	...
	public SubstitutionDecryptPanel() {
		...
		top.setBorder(BorderFactory.createTitledBorder(topBD, "Thông tin - Giải mã Substitution"));
		...
		ActionListener buttonListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (e.getActionCommand().equals("Choose File")) {
					...
					int userChoice = textFileChooser.showOpenDialog(SubstitutionDecryptPanel.this);
				...
				if(e.getActionCommand().equals("Load key")) {
					...
					int userChoice = keyFileChooser.showOpenDialog(SubstitutionDecryptPanel.this);
					if(userChoice == JFileChooser.APPROVE_OPTION) {
						keyFile = keyFileChooser.getSelectedFile();
						
						Substitution sub = new Substitution();
						try {
							subKey = sub.readKey(keyFile);
						} catch (FileNotFoundException e1) {
							e1.printStackTrace();
						} catch (IOException e1) {
							e1.printStackTrace();
						}
						keyTextField.setText(String.valueOf(subKey));
						keyUploaded = true;
					}
				...
				if(e.getActionCommand().equals("Decrypt")) {
					if(ctTextField.getText().isBlank() ||
						...
						int userChoice = dirChooser.showSaveDialog(SubstitutionDecryptPanel.this);
						if(userChoice == JFileChooser.APPROVE_OPTION) {
							...
							Substitution sub = new Substitution();
							try {
								sub.decrypt(text, key, destDir);
								
								//Show result to Text Area
								txtArea.setText(sub.getDecryptedString());
							} catch (Exception e1) {
								e1.printStackTrace();
							}
						}
